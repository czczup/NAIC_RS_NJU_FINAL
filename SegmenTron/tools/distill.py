...
class Trainer(object):
    def __init__(self, args):
        ...
        data_kwargs = {'transform': input_transform, 'base_size': cfg.TRAIN.BASE_SIZE,
                       'crop_size': cfg.TRAIN.CROP_SIZE, 'extra_data_probability': cfg.SOLVER.EXTRA_DATA_PROBABILITY}
    ...
    def train(self):
        ...
        for (images, targets_8, targets_14, logits_8, logits_14) in self.train_loader:
            ...
            logits_8 = logits_8.to(self.device, non_blocking=True)
            logits_14 = logits_14.to(self.device, non_blocking=True)

            outputs = self.model(images)
            loss_dict = self.criterion(outputs, tuple([targets_8, targets_14]), tuple([logits_8, logits_14]))
            ...
            if iteration % log_per_iters == 0 and self.save_to_disk:
                logging.info(
                    ...
                    "|| Distill-Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
    ...
    def validation(self, epoch):
        ...
        for i, (image, target_A, target_C) in enumerate(self.val_loader):
            ...
            with torch.no_grad():
                ...
                output_A = F.interpolate(output_A, size=(256, 256), mode='bilinear', align_corners=True)
                output_C = F.interpolate(output_C, size=(256, 256), mode='bilinear', align_corners=True)
