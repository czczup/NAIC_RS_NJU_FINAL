...
from segmentron.models.model_zoo import get_segmentation_model, get_segmentation_teacher_model
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.student = get_segmentation_model().to(self.device)
        logging.info("Building teacher model...")
        self.teacher = get_segmentation_teacher_model().to(self.device)
        self.teacher.eval()
        # print params and flops
        if get_rank() == 0:
            try:
                show_flops_params(copy.deepcopy(self.student), args.device)
        ...
        self.optimizer = get_optimizer(self.student)
        ...
        if args.resume and os.path.isfile(args.resume):
            ...
            self.student.load_state_dict(resume_sate['state_dict'])
        ...
        if cfg.SOLVER.USE_FP16 == True:
            self.student, self.optimizer = apex.amp.initialize(self.student.cuda(), self.optimizer, opt_level="O1")
            self.teacher = apex.amp.initialize(self.teacher.cuda(), opt_level="O3")
            logging.info("Initializing mixed precision done.")

        if args.distributed:
            self.student = nn.parallel.DistributedDataParallel(self.student, device_ids=[args.local_rank],
                                                               output_device=args.local_rank,
                                                               find_unused_parameters=True)
            self.teacher = nn.parallel.DistributedDataParallel(self.teacher, device_ids=[args.local_rank],
                                                               output_device=args.local_rank,
                                                               find_unused_parameters=True)
        else:
    ...
    def train(self):
        ...
        for (images, targets_8, targets_14) in self.train_loader:
            ...
            targets_14 = targets_14.to(self.device, non_blocking=True)
            ...
            if cfg.SOLVER.USE_FP16 == True:
                ...
                losses.backward()
            nn.utils.clip_grad_norm_(self.student.parameters(), max_norm=35, norm_type=2)
            ...
            if iteration % log_per_iters == 0 and self.save_to_disk:
                logging.info(
                    "Epoch: {:d}/{:d} || Iters: {:d}/{:d} || Lr: {:.6f} || CE-Loss: {:.4f} "
                    "|| PI-Loss: {:.4f} || PA-Loss: {:.4f} || Cost Time: {} || Estimated Time: {}".format(
                        epoch, epochs, iteration % iters_per_epoch, iters_per_epoch,
                        self.optimizer.param_groups[0]['lr'], loss_dict['ce_loss'].item(),
                        loss_dict['pi_loss'].item(), loss_dict['pa_loss'].item(),
                        str(datetime.timedelta(seconds=int(time.time() - start_time))),
                        eta_string))
                
            if iteration % self.iters_per_epoch == 0 and self.save_to_disk:
                save_checkpoint(self.student, epoch, self.optimizer, self.lr_scheduler, is_best=False)
    ...
    def validation(self, epoch):
        ...
        table = []
        for i, cls_name in enumerate(self.classes_A):
            table.append([cls_name, category_iou_A[i]])
        logging.info('DatasetA Category iou: \n {}'.format(tabulate(table, headers, tablefmt='grid', showindex="always",
                                                           numalign='center', stralign='center')))
        table = []
        for i, cls_name in enumerate(self.classes_C):
            table.append([cls_name, category_iou_C[i]])
        logging.info('DatasetC Category iou: \n {}'.format(tabulate(table, headers, tablefmt='grid', showindex="always",
                                                           numalign='center', stralign='center')))
