...
def save_checkpoint(model, epoch, optimizer=None, lr_scheduler=None, is_best=False):
    ...
    directory = os.path.join(directory, cfg.DIR_NAME)
    ...
    if is_best:
        ...
        if "1.6.0" in torch.__version__:
            torch.save(model_state_dict, best_filename, _use_new_zipfile_serialization=False)
        else:
        ...
        if not os.path.exists(filename):
            if "1.6.0" in torch.__version__:
                torch.save(save_state, filename, _use_new_zipfile_serialization=False)
            else:
