...
class Bottle2neck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, baseWidth=26,
                 scale=4, dilation=1, stype='normal'):
...
class Res2Net(nn.Module):
    ...
    def __init__(self, block, layers, baseWidth=26, scale=4, num_classes=1000, strides=None, dilations=None):
        self.inplanes = 64
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        layers.append(block(self.inplanes, planes, stride, downsample=downsample,
                            stype='stage', baseWidth=self.baseWidth, scale=self.scale, dilation=dilation))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, baseWidth=self.baseWidth, scale=self.scale, dilation=dilation))
...
def res2net50(norm_layer=nn.BatchNorm2d):
    model = Res2Net(Bottle2neck, [3, 4, 6, 3], baseWidth=26, scale=4, dilations=[1, 1, 2, 4], strides=[1, 2, 1, 1])
    return model
