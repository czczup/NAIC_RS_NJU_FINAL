...
class BiSeNet(SegBaseModel):
    ...
    def __init__(self):
        ...
        self.head = _BiSeHead(256, 64, 8)
        self.head2 = _BiSeHead(256, 64, 14)

        if self.aux:
            self.auxlayer1 = _BiSeHead(128, 256, 8)
            self.auxlayer2 = _BiSeHead(128, 256, 8)
            self.auxlayer3 = _BiSeHead(128, 256, 14)
            self.auxlayer4 = _BiSeHead(128, 256, 14)
    ...
    def forward(self, x):
        ...
        if self.aux:
            ...
            outputs1.append(auxout2)

            auxout3 = self.auxlayer3(context_out[0])
            auxout3 = F.interpolate(auxout3, size, mode='bilinear', align_corners=True)
            outputs2.append(auxout3)
            auxout4 = self.auxlayer4(context_out[1])
            auxout4 = F.interpolate(auxout4, size, mode='bilinear', align_corners=True)
            outputs2.append(auxout4)
