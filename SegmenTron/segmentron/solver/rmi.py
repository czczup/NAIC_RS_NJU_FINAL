...
class RMILoss(nn.Module):
    ...
    def forward(self, logits_4D, labels_4D, do_rmi=True):
        ...
        if isinstance(logits_4D, tuple):
            logits_4D = logits_4D[0]
        masked_indices = labels_4D.eq(-1)
        labels_4D.masked_fill_(masked_indices, self.ignore_index)
        logits_4D.float()
        labels_4D.float()
        if cfg.SOLVER.USE_FP16:
